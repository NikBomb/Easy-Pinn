...
## Summary
Create a standalone executable that trains Problem 0 over x in [0,1] using existing Problem 0 infrastructure, and writes residual-per-epoch and solution values over [0,1] (including exact solution and error) to CSV files in `results/`.
...
- R2: The app trains Problem 0 over x in [0,1] using the existing trial/residual/loss and trainer.
...
- R5: The app writes residual values per epoch to a CSV file in `results/`.
- R6: The app writes solution values y(x), exact values exp(x), and errors over [0,1] to a CSV file in `results/`.
...
- R8: No automated tests are required for this spec.
 ...
 - R11: Output filenames are `results/problem0_residuals.csv` and `results/problem0_solution.csv`.
